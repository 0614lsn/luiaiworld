...
import { ROOT, prepare, status, record, verifyRelease, releasePlatforms } from './core.mjs';
...
try {
  let result;
  if (command === 'prepare' && slug) {
    if (!values.platforms) throw new Error('生成前请先明确目标平台：--platforms website,wechat,xiaohongshu（任选一至三个）');
    result = await prepare(ROOT, slug, { platforms: values.platforms.split(',').map(value => value.trim()) });
  }
    ...
    const pack = await verifyRelease(ROOT, slug, hash);
    if (!releasePlatforms(pack.manifest).includes('website')) throw new Error('此版本未包含网站稿，请先选择 website 生成本地包');
    ...
    throw new Error('用法：content prepare <id> [--platforms website,wechat,xiaohongshu]；status <id>；verify <id> --release <hash>；preview <id> [--port 4322]；record <id> --release <hash> --platform <platform> --stage <stage> --target <目标> --evidence <证据>。record 仅登记已观察事实，不执行上传或发布。');
  }
