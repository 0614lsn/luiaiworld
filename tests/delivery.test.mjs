...
async function fixture(t, options = {}) {
  ...
  await writeFile(join(directory, 'xiaohongshu.json'), JSON.stringify({ sourceHash: sha256(article), title: '交付测试', caption: '第一段。\n\n第二段。', cards: [{ kicker: '测试', heading: '三站草稿', summary: '可验证内容', prompt: '正文摘录', note: '测试来源' }] }));
...
test('one delivery prepares three representations; double click reuses the job and leaves production alone', async t => {
  ...
  const [a,b] = await Promise.all([f.service.start('example', user), f.service.start('example', user)]);
...
test('save response loss survives process restart; resume requires readback and cannot save a second time', async t => {
  const f = await fixture(t), user=target(); const job=await ready(f.service,await f.service.start('example',user));
  const permit=await f.service.xhsAttempt(job.id,user.instanceId);
  const restarted=new DeliveryService(f.root,{wechat:async()=>({stage:'draft_saved'})});
  const resumed=await restarted.start('example',user); assert.equal(resumed.id,job.id); await ready(restarted,resumed);
...
test('unknown old-version save can be reconciled by job ID after the source changes',async t=>{
  const f=await fixture(t),user=target(),job=await ready(f.service,await f.service.start('example',user));
  const permit=await f.service.xhsAttempt(job.id,user.instanceId);
...
test('content, image order, profile and partial failures cannot be reported as all delivered', async t => {
  ...
  const job=await ready(f.service,await f.service.start('example',user));
  ...
  const other=await f.service.start('example',target()); assert.notEqual(other.id,job.id); await ready(f.service,other);
});
test('lossless re-encoding requires exact decoded pixels; one changed pixel still fails',async t=>{
  const f=await fixture(t),job=await ready(f.service,await f.service.start('example',target()));
...
test('repeating a completed delivery rechecks browser-local storage; manual website edits are protected', async t=>{
  ...
  const next=await f.service.start('example',user); const refreshed=await ready(f.service,next);
  ...
  const again=await f.service.start('example',user); await f.service.active.get(again.id);
